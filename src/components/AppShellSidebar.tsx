...
export function AppShellSidebar({
    ...
    return (
        ...
        fixed lg:sticky
        lg:top-0
        inset-y-0 left-0 z-40 w-72

        transform transition-transform duration-300 ease-in-out
        ${open ? "translate-x-0" : "-translate-x-full"}
        lg:translate-x-0

        bg-white lg:bg-sidebar
        text-slate-900 lg:text-sidebar-foreground

        border-r border-slate-200 lg:border-sidebar-border
    ...
    `}
            >
                {/* HEADER */}
                <div className="flex-shrink-0 h-16 flex items-center justify-between px-5 border-b border-slate-100 lg:border-sidebar-border bg-white">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-[#facc15]/15 to-[#2563eb]/15 border border-slate-200 grid place-items-center overflow-hidden shadow-sm">
                    ...
                    <button
                        onClick={() => setOpen(false)}
                        className="lg:hidden text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-slate-100 transition"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                {/* FOOTER - Siempre al final sin scroll */}
                <div className="flex-shrink-0 p-3 border-t border-slate-100 lg:border-sidebar-border bg-white/80 backdrop-blur-sm">
                    {/* Usuario */}
